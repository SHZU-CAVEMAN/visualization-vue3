...
const  getSeriesData = () => {
	...
	props.data.abnormals.forEach((item, index) => {
		...
		series.push({
			name: item.name, 
			type: 'pie', // 使用饼图绘制，减少饼图宽度即为环形图
			clockWise: false, // 逆时针排布
			hoverAnimation: false, // 不展示鼠标移入动画
			radius: [73 - index * 15 + '%', 68 - index * 15 + '%'], // 半径位置，需要依次递减，否则会重复在一处进行展示
			center: ['55%', '55%'], // 中心点
			label: { show: false }, // 不展示 label
			data: [ // 数据配置
				{ value: item.value, name: item.name }, // 设置数据与名称
				// 最大数据，展示比例
				{
					value: 1000,
					name: '',
